    ...
    DI: DisplayInterface,
{
    ...
    pub async fn init_column_mode(&mut self) -> Result<(), DI::Error> {
        self.iface.init()?;
        ...
        match self.display_size {
            DisplaySize::Display64x128 => Command::DisplayOffset(0x62).send(&mut self.iface).await,
            DisplaySize::Display128x128
            | DisplaySize::Display128x32
            | DisplaySize::Display128x64
            | DisplaySize::Display128x64NoOffset
            | DisplaySize::Display132x64 => Command::DisplayOffset(0).send(&mut self.iface).await,
        }?;
    ...
    pub async fn set_draw_area(&mut self, start: (u8, u8), end: (u8, u8)) -> Result<(), DI::Error> {
    ...
    /// this method.
    pub async fn draw(&mut self, mut buffer: &[u8]) -> Result<(), DI::Error> {
        while !buffer.is_empty() {
            let count = self.draw_area_end.0 - self.draw_column;
            self.iface.send_data(&buffer[..count as usize]).await?;
    ...
    async fn send_draw_address(&mut self) -> Result<(), DI::Error> {
    ...
    pub async fn set_rotation(&mut self, display_rotation: DisplayRotation) -> Result<(), DI::Error> {
